...
class MyDocumentHTMLDecoder extends Converter<String, Document> {
  ...
  Iterable<Node> transform(Iterable<Node> nodes) {
    final List<Node> updatedNodes = [];
    for (final node in nodes) {
      if (listTypes.contains(node.type)) {
        updatedNodes.add(node.copyWith(children: []));
        visitAllDescendants(
          node,
          (child, indent) {
            updatedNodes.add(child.copyWith(indent: indent, children: []));
          },
          0,
        );
      } else {
